        ...
        <Base className='container p-4 bg-info' title='Manage categories' description='You can Handle the categories operations'>
            <div className="row">
                <div className="col-md-8 offset-md-2 bg-white text-dark p-4">
                    ...
                    {success ? <Alert alerttype="success" msg={`Product ${deletedcategory} deleted successfully`} /> : error ? <Alert alerttype='danger' msg={errormsg} /> : null}
                        ...
                        <tbody>
                            {
                                categories.map((category, index) => (
                                    <tr key={category._id}>
                                        ...
                                        <td>
                                            <div>
                                                <button onClick={() => deleteHandler(category._id)} className="btn btn-danger m-2">Delete</button>
                                                <Link to={`/admin/category/updateCategory/${category._id}`} className="btn btn-primary m-2">Update</Link>
                                            </div>
                                        </td>
                                    </tr>
                                ))
