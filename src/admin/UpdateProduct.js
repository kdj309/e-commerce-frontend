...
import { getCategories, getCategory, getproduct, updateProduct } from './helper/adminapicall';
...
export default function UpdateProduct({ match }) {
    ...
    const { name, category, categories, description, price, Availabelstock, size, image, loading, error, errormsg, formData, UpdatedProduct, success } = productfields
    ...
    const onSubmit = ((e) => {
        ...
        updateProduct(id, authtoken, match.params.productid, formData).then((data) => {
            if (data.errormsg) {
                setproductfields({
                    ...productfields,
                    error: true,
        ...
        <Base className='container p-4 bg-info' title='Manage categories' description='You can Handle the categories operations'>
            <div className="row">
                <div className="col-md-8 offset-md-2 bg-white text-dark p-4">
                    ...
                    {success ? <Alert alerttype="success" msg={`Product ${UpdatedProduct} updated successfully`} /> : error ? <Alert alerttype='danger' msg={errormsg} /> : null}
